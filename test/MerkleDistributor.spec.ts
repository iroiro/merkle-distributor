  ...
  describe('#claim', () => {
    ...
    describe('two account tree', () => {
      ...
      it('gas', async () => {
        ...
        const tx = await distributor.claim(0, await wallet0.getAddress(), 100, proof, overrides)
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(80228)
      })
    })
    ...
    describe('larger tree', () => {
      ...
      it('gas', async () => {
        const proof = tree.getProof(9, await wallets[9].getAddress(), BigNumber.from(10))
        const tx = await distributor.claim(9, await wallets[9].getAddress(), 10, proof, overrides)
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(83572)
      })
      ...
      it('gas second down about 15k', async () => {
        ...
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(68572)
      })
    })
    ...
    describe('realistic size tree', () => {
      ...
      it('gas', async () => {
        const proof = tree.getProof(50000, await wallet0.getAddress(), BigNumber.from(100))
        const tx = await distributor.claim(50000, await wallet0.getAddress(), 100, proof, overrides)
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(94423)
      })

      it('gas deeper node', async () => {
        const proof = tree.getProof(90000, await wallet0.getAddress(), BigNumber.from(100))
        const tx = await distributor.claim(90000, await wallet0.getAddress(), 100, proof, overrides)
        const receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq(95261)
      })
      it('gas average random distribution', async () => {
        ...
        for (let i = 0; i < NUM_LEAVES; i += NUM_LEAVES / NUM_SAMPLES) {
          ...
          const tx = await distributor.claim(i, await wallet0.getAddress(), 100, proof, overrides)
          const receipt = await tx.wait()
          total = total.add(receipt.gasUsed)
          count++
        }
        const average = total.div(count)
        expect(average).to.eq(80690)
      })
